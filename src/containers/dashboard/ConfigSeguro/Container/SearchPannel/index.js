...
import { updatePesquisa, pesquisarSeguroRequest } from 'modules/pesquisar/seguro/actions';
...
const SearchPannel = ({ expanded, handleChange }) => {
  ...
  const {
    canaisSeguro: { canaisSeguroDisponiveis },
    pesquisarSeguro: { canal, pdv, seguroMovel, impedidoBuscaCanal, pdvs },
  } = useSelector(store => store);
  const listCanaisDisponiveis = [{ label: 'Selecione', value: 'SELECIONE' }].concat(
    canaisSeguroDisponiveis.map(({ canal: canalLabel, canalId }) => ({
      label: canalLabel,
      value: canalId,
    })),
  );
  ...
  const updateseguroMovel = updateField('seguroMovel');
